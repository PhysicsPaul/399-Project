...
def evolve(central_mass, num_threads, length, length_units, resol, duration, duration_units, step_factor, save_number, save_options,
    ...
    for i in range(3, 6):
        if tm[i] in range(0, 10):
            ...
            talt[i - 3] = tm[i]
    timestamp = '{}{}{}{}{}{}{}{}{}{}{}{}{}'.format(tm[0], '.', tm[1], '.', tm[2], '_', talt[0], '__', talt[1], '_', talt[2], '_', resol)
    print(timestamp)
    file = open('{}{}{}'.format('./', save_path, '/timestamp.txt'), "w+")
    file.write(timestamp)
    os.makedirs(f'./{save_path}/{timestamp}',exist_ok=True)
    ...
    phik = ne.evaluate(f"-4*{np.pi}*phik/rkarray2")
    phik[0, 0, 0] = 0
